...
| Explorer window open but NOT foreground | pill adds nothing measurable — a 2 s liveness/placement tick with no shell calls and no disk reads (see the isolation measurement below) | core + ~80 KB per pill |
...
- **Pill gate (E2)**: the shell enumeration and the folder read run only while an Explorer window is foreground (700 ms tick), plus a short grace window after Explorer gains focus (a window that just opened is not enumerable for a moment). An unfocused Explorer window costs a 2 s tick that makes no shell call and touches no disk, and with no Explorer window there is no timer at all. Nothing that can change a count while the user looks elsewhere is missed: navigation and tab switches require focus, a new or closed window is caught by a class-only `EnumWindows` scan, and the editor pokes `pill::notes_changed()` when a note is saved or deleted. **The foreground hook filters on `CabinetWClass` in the callback**: an unfiltered version opened the grace window on every foreground change on the desktop and that alone was measurable.
