...
| Explorer window open but NOT foreground | ~0% (measured 15.6 ms over 25 s, debug build: a 2 s liveness/placement tick with no shell calls and no disk reads) | core + ~80 KB per pill |
| No Explorer window open | ~0% (no pill, no pill timer at all) | core |
...
- **Pill gate (E2)**: no Explorer window ⇒ no pill and no timer (measured 0 ms over 20 s). The shell enumeration and the folder read run only while an Explorer window is foreground (700 ms tick); an unfocused Explorer window costs a 2 s liveness/placement tick that makes no shell call and touches no disk. Nothing that can change a count while the user looks elsewhere is missed: navigation and tab switches require focus, a new or closed window arrives as a foreground event, and the editor pokes `pill::notes_changed()` when a note is saved or deleted.
