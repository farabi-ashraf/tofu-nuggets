...
| **Desktop-vs-window routing (macOS)** | Per hit, the surface is decided by AX *shape*, not size: an `AXWindow` in the hit's ancestor chain ⇒ a Finder browser window (folder from that window's `AXDocument`); none ⇒ the desktop (the desktop has no `AXWindow`). Replaces the old display-size heuristic that mis-read a maximized icon-view window as the desktop (M5 fix). |
| **Active tab (Finder)** | Finder renders only the active tab (inactive tabs are absent from the AX tree) and keeps the window's `AXDocument` pointed at the active tab's folder, so a multi-tab Finder window resolves the front tab's folder directly — no cursor-in-view disambiguation, unlike Explorer's live per-tab HWNDs. |
...
| `desktop_mac.rs` | **macOS** `DesktopIcons` impl: AX hit-test hover over the desktop **and Finder browser windows** (M5), gated by `finder_frontmost`, routed desktop-vs-window by `AXWindow` in the hit chain, Finder-window folder from `AXDocument` (active tab), hidden-extension-safe name→path via `resolve_path`; `list_icons`/`selected_icon` (desktop + Finder-window selection) by walking down from Finder's app element (pid from the CG window list); hand-declared FFI, points throughout, Accessibility prompt/status, `debug_cursor_chain` + `debug_finder_tree` dumps (gate/route/`AXDocument`) for the log | `MacIcons`, `debug_cursor_chain` |
